...
export class EnsProvider extends ProviderClient {
    ...
    async resolveDid(address: string, textRecordFields?: string[]): Promise<{data: any | null, error: any | null}> {
        ...
        if (textRecordFields) {
            ...
            try {
                const textRecordValues = await Promise.all(textRecordFields.map(f => resolver.getText(f) || null))
    ...
    private async _resolverForDomain(domain: string): Promise<{resolver: Resolver | null, error: any | null}> {
        try {
    ...
    private async _resolveAvatarUrl(url: string): Promise<string | null> {
        ...
        if(url.startsWith(protocols.ERC721)) {
            return await this._getAvatarUrlFromTokenContract(url, tokenUriAbis.ERC721, tokenUriGetters.ERC721)
        }
        // ERC1155 url.
        if (url.startsWith(protocols.ERC1155)) {
            return await this._getAvatarUrlFromTokenContract(url, tokenUriAbis.ERC1155, tokenUriGetters.ERC1155)
        }
        return null
    }
